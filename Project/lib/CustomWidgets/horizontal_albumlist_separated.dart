...
class HorizontalAlbumListSeparated extends StatelessWidget {
  final List songsList;
  final Function(int) onTap;
  const HorizontalAlbumListSeparated(
  ...
  String getSubTitle(Map item) {
    ...
    if (type == 'charts') {
      ...
      final artist = item['more_info']?['artistMap']?['artists']
          .map((artist) => artist['name'])
          .toList();
      if (artist != null) {
        return formatString(artist?.join(', ')?.toString());
      }
      if (item['artist'] != null) {
        return formatString(item['artist']?.toString());
      }
